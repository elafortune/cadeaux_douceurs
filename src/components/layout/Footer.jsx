      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-14 md:py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 md:gap-6">
          ...
          <div className="flex flex-col gap-4 md:col-span-1">
            ...
            <p className="font-sans text-sm leading-relaxed text-stone/55 max-w-xs">
              Broderie artisanale sur tous supports textiles à Montpellier.
              Personnalisation unique pour chaque occasion.
            </p>
            <p className="font-sans text-xs text-stone/35">
            ...
            <div className="flex items-center gap-3 mt-2">
              <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram Cadeaux Douceurs"
              ...
              <a href={socialLinks.tiktok} target="_blank" rel="noopener noreferrer" aria-label="TikTok Cadeaux Douceurs"
              ...
              <a href={socialLinks.phone} aria-label="Appeler Cadeaux Douceurs"
          ...
          <div className="flex flex-col gap-3">
            ...
            {horaires.map(({ jour, heure }) => (
              <div key={jour} className="flex justify-between gap-4 text-sm">
                ...
                <span className={`font-semibold ${heure === 'Fermé' ? 'text-stone/30' : 'text-white/80'}`}>
                  {heure}
                </span>
              </div>
            ))}
          ...
          <div className="flex flex-col gap-4">
            ...
            <a href={socialLinks.phone} className="flex items-center gap-2 text-stone/50 hover:text-white transition-colors text-sm">
              <Phone size={16} className="text-warm/70" />
              <span>Par téléphone</span>
            </a>
            <a href="mailto:rosarita.lafortune@gmail.com" className="flex items-center gap-2 text-stone/50 hover:text-white transition-colors text-sm">
              <span className="text-rose/70 text-xs">✉</span>
              <span>Par email</span>
            </a>
            <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-stone/50 hover:text-white transition-colors text-sm">
              <Instagram size={16} className="text-rose/70" />
              <span>Instagram</span>
            </a>
            <a href={socialLinks.tiktok} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-stone/50 hover:text-white transition-colors text-sm">
        ...
        <div className="mt-12 pt-8 border-t border-white/8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="font-sans text-xs tracking-widest uppercase text-stone/25">
            &copy; {new Date().getFullYear()} Cadeaux Douceurs — Montpellier
          </p>
          <p className="font-sans text-xs text-stone/20 italic">
            Broderie artisanale &bull; Personnalisation textile &bull; Hérault (34)
          </p>
        </div>
      </div>
    </footer>
